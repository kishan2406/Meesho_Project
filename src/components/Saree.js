...
function  Saree() {
  ...
  const getdata = async () =>{
      const data = await fetch (" https://meesho-all.herokuapp.com/ethnicwear").then((d)=>d.json()
      )
      setstudent(data)
  }
    ...
    <div className="pro_con"  key="lk">
    {
      ...
      <div className="single_pro">
        ...
        <p className="productbox">{e.title}</p>
         ...
         <h2 className="org_pri"> <i className="fa-solid fa-indian-rupee-sign"></i>{e.price} </h2>
          <h4 className="discount"><i className="fa-solid fa-indian-rupee-sign"></i>{e.d_price}</h4>
           <p className="dis10">10% discount</p>
         </div>
         <p> <img src="https://img.icons8.com/ios-glyphs/20/000000/discount--v1.png " alt="error"/> <i className="fa-solid fa-indian-rupee-sign"></i>50 discount on  1st order</p>
         <div className="freee">
